...
class ProcessadorCVPSportal:
    """Classe para processar dados de CVP (SPORTAL)"""
    
    def __init__(self):
        self.nome_arquivo_final = f"3-CVP-SPORTAL-{datetime.now().year}-QGP.xlsx"
    ...
    def processar_dados(self, df, data_inicio, data_fim):
        """Processa os dados de CVP (SPORTAL)"""
        try:
...
def interface_cvp_sportal():
    """Interface principal do módulo CVP (SPORTAL)"""
    st.title("📊 CVP (SPORTAL) - Crimes Violentos contra o Patrimônio")
    st.markdown("### 🔍 Processamento de dados SPORTAL")
    st.divider()
    
    processador = ProcessadorCVPSportal()
    
    st.subheader("📁 Upload do Arquivo")
    arquivo_uploaded = st.file_uploader(
        "Selecione o arquivo CVP do SPORTAL (Excel/CSV)",
        type=['xlsx', 'xls', 'csv'],
        help="Faça upload do arquivo extraído do sistema SPORTAL"
    )
