...
def render() -> None:
    ...
    st.caption(
        "Execução em fila dos 10 indicadores, com validação por nome de arquivo, "
        "processamento individual e download consolidado em ZIP."
    )
    ...
    arquivos_consolidados = st.file_uploader(
        "Arquivos consolidados dos indicadores",
        type=["xlsx", "xls"],
    ...
    for arquivo in arquivos_consolidados or []:
        ...
        if indicador.chave in uploads_identificados:
            conflitos.append(
                f"{indicador.titulo}: '{uploads_identificados[indicador.chave].name}' e '{arquivo.name}'"
            )
            continue
    ...
    pendentes = [
        indicador.titulo
        for indicador in INDICADORES
        if indicador.chave not in uploads_identificados
    ]
    ...
    for indicador in INDICADORES:
        ...
        if status == "OK":
            st.success(f"{indicador.ordem}. {indicador.titulo}: {consolidado}")
        else:
            st.warning(f"{indicador.ordem}. {indicador.titulo}: aguardando arquivo")
