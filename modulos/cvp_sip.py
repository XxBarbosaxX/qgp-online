"""
Módulo CVP (SIP) - Crimes Violentos contra o Patrimônio
Processamento e atualização de dados CVP do sistema SIP para QGP Online
"""
...
class ProcessadorCVPSip:
    """Classe para processar dados de CVP (SIP)"""
    
    def __init__(self):
        self.nome_arquivo_final = f"3-CVP-SIP-{datetime.now().year}-QGP.xlsx"
    ...
    def atualizar_base(self, df_base, df_novo, coluna_data):
        ...
        # Remover registros da base que estejam nos mesmos meses/anos do arquivo novo
...
def interface_cvp_sip():
    """Interface Streamlit para processamento CVP SIP"""
    st.markdown("### Processamento CVP (SIP)")
    st.markdown("Atualize a base de Crimes Violentos contra o Patrimônio do sistema SIP")
    
    col1, col2 = st.columns(2)
    
    with col1:
        arquivo01 = st.file_uploader(
            "📁 Arquivo 01 - Base de dados CVP SIP",
            type=["xlsx", "xls"],
            key="cvp_sip_arquivo01"
        )
    ...
    with col2:
        arquivo02 = st.file_uploader(
            ...
            type=["xlsx", "xls"],
            key="cvp_sip_arquivo02"
        )
    
    salvar_drive = st.checkbox("💾 Salvar no Google Drive", key="cvp_sip_drive")
    
    if st.button("▶️ Processar CVP SIP", key="processar_cvp_sip"):
        if not arquivo01:
        ...
        with st.spinner("Processando dados CVP SIP..."):
            processador = ProcessadorCVPSip()
        ...
        if resultado['sucesso']:
            ...
            st.info(f"📊 **{resultado['adicionados']}** CVPs novos adicionados")
            st.info(f"📈 Total de **{resultado['total_final']}** CVPs na base")
            st.info(f"🔄 Arquivo {acao} com sucesso")
            
            # Gerar arquivo para download
            output = io.BytesIO()
            with pd.ExcelWriter(output, engine='openpyxl') as writer:
                resultado['df_final'].to_excel(writer, index=False, sheet_name='CVP_SIP')
