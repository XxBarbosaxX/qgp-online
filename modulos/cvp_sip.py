...
def processar_cvp_sip(arquivo_01, arquivo_02):
    ...
    df_novo["__datahora__"] = pd.to_datetime(df_novo[col_datahora_novo], errors="coerce", dayfirst=True)
...
def render():
    ...
    arquivo_01 = st.file_uploader(
        "Arquivo 01 - Base histórica CVP",
        type=["xlsx", "xls"],
        key="cvp_sip_arquivo_01",
    )

    arquivo_02 = st.file_uploader(
        "Arquivo 02 - Complemento SIP",
        type=["xlsx", "xls"],
        key="cvp_sip_arquivo_02",
    )

    if st.button("Processar CVP (SIP)", type="primary"):
        if not arquivo_01 or not arquivo_02:
            st.warning("Selecione os dois arquivos para continuar.")
            return

        try:
            with st.spinner("Processando e geocodificando registros..."):
                df_final, resumo = processar_cvp_sip(arquivo_01, arquivo_02)
