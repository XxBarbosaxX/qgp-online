"""
Modulo Furto de Veiculo (SPORTAL)
...
def _normalizar_nome_aba(nome: str) -> str:
    ...
    return normalizado.upper().strip().replace(" ", "").replace("_", "").replace("-", "")


def _selecionar_aba_arquivo_02(sheet_names: list[str]) -> str:
    alvo_exato = "FURTOCIOPS"
    ...
    for aba in sheet_names:
        ...
        if "FURTO" in nome and "CIOPS" in nome:
            return aba

    raise ValueError(
        f"Aba 'Furto CIOPS' nao encontrada no Arquivo 02. "
...
def processar_furto_veiculo_sportal(arquivo_01, arquivo_02):
    arquivo_01.seek(0)
    arquivo_02.seek(0)
    ...
    if df_novo.empty:
        raise ValueError(
            "Apos filtrar 'Nome da Ocorrencia' por 'FURTO DE VEICULO' e excluir "
            "'BICICLETA' e 'BICICLETA DE APLICATIVO' em 'Subnome da Ocorrencia', "
    ...
    resumo = {
        ...
        "removidos_por_filtro_ocorrencia": removidos_por_filtro_ocorrencia,
...
def _init_state():
    defaults = {
        "furto_veiculo_sportal_arquivo_01_bytes": None,
        "furto_veiculo_sportal_arquivo_01_nome": None,
        "furto_veiculo_sportal_arquivo_02_bytes": None,
        "furto_veiculo_sportal_arquivo_02_nome": None,
        "furto_veiculo_sportal_resultado_excel": None,
        "furto_veiculo_sportal_resultado_df": None,
        "furto_veiculo_sportal_resumo": None,
    }
...
def render():
    ...
    st.subheader("Furto de Veiculo (SPORTAL)")
    st.write(
    ...
    if arquivo_01 is not None:
        arquivo_01.seek(0)
        st.session_state.furto_veiculo_sportal_arquivo_01_bytes = arquivo_01.read()
        st.session_state.furto_veiculo_sportal_arquivo_01_nome = arquivo_01.name
    ...
    if st.button("Processar Furto de Veiculo (SPORTAL)", type="primary", disabled=not pode_processar):
        try:
    ...
    if (
        ...
        st.info(
            ...
            f"Removidos por filtro de ocorrencia: {resumo['removidos_por_filtro_ocorrencia']}"
        )

        st.info(
            f"Removidos por coordenadas invalidas: {resumo['removidos_invalidos']} | "
