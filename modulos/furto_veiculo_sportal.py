...
class ProcessadorFurtoVeiculoSportal:
    def __init__(self):
        self.nome_arquivo_final = f"8-FURTO-VEICULO-SPORTAL-{datetime.now().year}-QGP.xlsx"
    ...
    def gerar_arquivo_excel(self, df):
        ...
        with pd.ExcelWriter(output, engine='openpyxl') as writer:
            df.to_excel(writer, index=False, sheet_name='FURTO_VEICULO_SPORTAL')
        return output.getvalue()

def interface_furto_veiculo_sportal():
    st.title("🚗 Furto de Veículo (SPORTAL)")
    st.markdown("### 🔍 Processamento de dados SPORTAL")
    st.divider()
    processador = ProcessadorFurtoVeiculoSportal()
