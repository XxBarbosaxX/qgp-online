"""Módulo Roubo de Veículo (SPORTAL)"""
import pandas as pd
import streamlit as st
from datetime import datetime
import io

class ProcessadorRouboVeiculoSportal:
    def __init__(self):
        self.nome_arquivo_final = f"4-ROUBO-VEICULO-SPORTAL-{datetime.now().year}-QGP.xlsx"
    ...
    def gerar_arquivo_excel(self, df):
        ...
        with pd.ExcelWriter(output, engine='openpyxl') as writer:
            df.to_excel(writer, index=False, sheet_name='ROUBO_VEICULO_SPORTAL')
        return output.getvalue()

def interface_roubo_veiculo_sportal():
    st.title("🚗 Roubo de Veículo (SPORTAL)")
    st.markdown("### 🔍 Processamento de dados SPORTAL")
    st.divider()
    processador = ProcessadorRouboVeiculoSportal()
