...
def converter_coordenadas_para_wgs84_auto(
    ...
    Zonas suportadas:
      - EPSG:31984 — SIRGAS 2000 / UTM zona 24S (Ceará, maior parte).
      - EPSG:31983 — SIRGAS 2000 / UTM zona 23S (faixa oeste do estado, fallback).
    """
    try:
        from pyproj import Transformer
    ...
    # Bounding box amplo para validação do resultado reprojetado (Brasil + margem)
    _LON_MIN, _LON_MAX = -75.0, -28.0
    _LAT_MIN, _LAT_MAX = -35.0, 6.0

    def _reprojetar(easting: float, northing: float) -> tuple[float, float] | None:
        """Tenta reprojetar via zona 24S; usa 23S como fallback. Retorna (lat, lon) ou None."""
    ...
    for y_raw, x_raw in zip(df[col_y_or_lat], df[col_x_or_lon]):
        ...
        # Caso 1: coordenadas já em graus decimais WGS84
        parecem_graus = (-90.0 <= y <= 90.0) and (-180.0 <= x <= 180.0)

        # Caso 2: UTM com ordem correta — col_y = Northing, col_x = Easting
        # Easting típico: 100.000 – 900.000 | Northing típico: 1.000.000 – 10.000.000
        parecem_utm_direto = (
            100_000 <= abs(x) <= 900_000
            and 1_000_000 <= abs(y) <= 10_000_000
        )

        # Caso 3: UTM com colunas invertidas — col_y = Easting, col_x = Northing
        # Ocorre quando o arquivo nomeia as colunas de forma ambígua
        ...
        if parecem_graus:
            ...
            # Corrige a inversão: trata col_y como Easting e col_x como Northing
            ...
            # Fora de qualquer faixa reconhecível
            lat_resultado.append(pd.NA)
            lon_resultado.append(pd.NA)
