...
Atenção para CVLI:
- O módulo cvli.py já faz toda a lógica temporal: identifica meses no arquivo complementar,
  remove os meses sobrepostos da base consolidada e insere os novos dados.
- Este orquestrador NÃO deve reusar o consolidado para CVLI, apenas receber o df_final
  do cvli.py e padronizar colunas caso necessário.
"""
...
def _pos_processar_cvli(df_final: pd.DataFrame) -> pd.DataFrame:
    ...
    - remove colunas auxiliares como Tombo, Delegacia, etc.;
    - renomeia AISNova -> AIS;
    - mantém apenas colunas taxativas na ordem.
    Não mexe em datas/períodos (já tratados em cvli.py).
    """
    df = df_final.copy()
    ...
    df = df.drop(columns=[c for c in df.columns if c in colunas_extras], errors="ignore")
...
def executar_modulo(
    ...
    # CVLI: usar resultado exato do módulo cvli.py
    if indicador.chave == "cvli":
        ...
        # Ajuste de layout apenas de colunas.
...
# restante do arquivo (UI, fila, zip) idêntico ao seu atual,
# sem nenhuma diferença de lógica, apenas reaproveitando executar_modulo.
