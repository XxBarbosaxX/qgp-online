...
def carregar_base_geografica():
    ...
    raise FileNotFoundError(f"Base parquet nao encontrada em: {CAMINHO_BASE_ENXUTA}")


def carregar_municipios():
    if os.path.exists(ARQ_CACHE_MUN):
        try:
            with open(ARQ_CACHE_MUN, encoding="utf-8") as f:
                return json.load(f)
        except Exception:
            pass
    ...
    try:
        ...
        with open(ARQ_CACHE_MUN, "w", encoding="utf-8") as f:
...
def processar_roubo_veiculo_sip(arquivo_01, arquivo_02):
    ...
    if not df_novo_util.empty:
        ...
        df_novo_util, geocodificados = geocodificar_linhas_novas(df_novo_util, col_lat_base, col_lon_base)
    ...
    resumo = {
        ...
        "base_geocodificacao": CAMINHO_BASE_ENXUTA,
    }
