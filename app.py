...
def load_custom_css():
    ...
    .custom-card h3 { color: #ffffff; font-size: 1.25rem; font-weight: 800; }
    .custom-card p  { color: #d5ddd8; font-size: 0.98rem; line-height: 1.65; }
    .stButton > button {
...
def render_topbar():
    st.markdown(f"""
    <div class="topbar">
        <img class="topbar-logo" src="{DIESP_SVG_B64}" alt="DIESP">
        <div>
...
with st.sidebar:
    ...
    st.markdown(f'<span class="metric-chip">Versao 1.0.0</span>', unsafe_allow_html=True)
    st.markdown(f'<span class="metric-chip">Data {datetime.now().strftime("%d/%m/%Y")}</span>', unsafe_allow_html=True)
    st.markdown(f'<span class="metric-chip">Hora {datetime.now().strftime("%H:%M:%S")}</span>', unsafe_allow_html=True)
...
with col1:
    ...
    <div class="custom-card">
        ...
        <p>Aplicacao web para atualizacao, tratamento e consolidacao dos indicadores de seguranca publica,
        com foco em padronizacao operacional, auditoria e escalabilidade da rotina tecnica.</p>
    </div>
    """, unsafe_allow_html=True)
with col2:
    st.markdown("""
    <div class="custom-card">
        ...
        <p>O sistema foi estruturado para receber planilhas, validar entradas, processar indicadores
        e integrar armazenamento automatizado com Google Drive e controle de execucao online.</p>
    </div>
    """, unsafe_allow_html=True)
with col3:
    st.markdown("""
    <div class="custom-card">
        <h3>Operacao QGP</h3>
        <p>Interface institucional com foco em produtividade, visual estrategico e organizacao
        do fluxo de trabalho para atualizacao das bases e geracao dos arquivos finais do QGP.</p>
    </div>
    """, unsafe_allow_html=True)
